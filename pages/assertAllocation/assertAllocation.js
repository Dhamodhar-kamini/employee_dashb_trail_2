...
async function loadAssets() {
    try {
        const response = await fetch("http://13.60.26.193:8000/api/assets/");
        if (response.ok) {
...
async function loadReturnAssets() {
    try {
        const response = await fetch("http://13.60.26.193:8000/api/return-assets/");
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    hraAssetForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            let url = "http://13.60.26.193:8000/api/assets/save/";
            let method = "POST";

            // If editing → update instead of create
            if(currentEditingDatabaseId){
                    url = `http://13.60.26.193:8000/api/assets/${currentEditingDatabaseId}/`;
                    method = "PATCH";
                }
    ...
    if(confirm("Are you sure you want to delete this asset?")) {
        ...
        const response = await fetch(`http://13.60.26.193:8000/api/assets/${dbId}/`,{
            method:"DELETE"
        });
...
if (button.classList.contains('hra-return-confirm-btn')) {
    ...
    const response = await fetch(`http://13.60.26.193:8000/api/return-status/${returnId}/`, {
        method: "PATCH",
        headers: {
...
if (button.classList.contains('hra-return-reject-btn')) {
    ...
    if(confirm("Delete this return record?")) {
        ...
        const response = await fetch(`http://13.60.26.193:8000/api/return-assets/${returnId}/`,{
            method:"DELETE"
        });
